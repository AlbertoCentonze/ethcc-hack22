...
contract VaultAave is Ownable, IVault {
    ...
    address private constant StkAave =
        0x4da27a545c0c5B758a6BA100e3a049001de870f5; // Todo: Same address as staking SC ??
    address private constant PalStkAave =
        0x24E79e946dEa5482212c38aaB2D0782F04cdB0E0;
    address private constant AaveStakingContract =
        0x4da27a545c0c5B758a6BA100e3a049001de870f5;
    address private constant PaladinPoolContract =
        0xCDc3DD86C99b58749de0F697dfc1ABE4bE22216d;
    address private constant PaladinController =
    ...
    function _depositStkAave(uint amount)
        private
        returns (uint palStkAaveAmount)
    {
    ...
    function _stakePalStkAave(uint amount) private {
        ...
        if (
            !IPaladinController(PaladinController).deposit(PalStkAave, amount)
        ) {
            revert GenericError();
        }
    }
    ...
    function _harvestAutocompounder(address to) private {
        _refreshInterestGenerated();
        uint amountToWithdraw = totalInterestGenerated /
            IPalPool(PaladinPoolContract).exchangeRateCurrent();
        uint256 underlyingReceived = IPalPool(PaladinPoolContract).withdraw(
            amountToWithdraw
        );
    ...
    function _refreshInterestGenerated() private {
        uint deltaRate = IPalPool(PaladinPoolContract).exchangeRateCurrent() -
            lastExchangeRate;
